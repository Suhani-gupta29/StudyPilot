        ...
        viewModelScope.launch {
            try {
                ...
                val targetDestination = withContext(Dispatchers.IO) {
                    ...
                    // User is logged in - try to get preferences with multiple retries
                    var prefs: com.example.studypilot.data.UserPreferences? = null
                    var retryCount = 0
                    val maxRetries = 3
                    ...
                    while (prefs == null && retryCount < maxRetries) {
                        ...
                        if (prefs == null) {
                            retryCount++
                            android.util.Log.d("SplashViewModel", "Prefs null on attempt $retryCount, waiting ${500L * retryCount}ms...")
                            delay(500L * retryCount) // Increasing delay: 500ms, 1000ms, 1500ms
                        }
                    }
