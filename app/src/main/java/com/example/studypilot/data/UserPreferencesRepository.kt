...
class UserPreferencesRepository(private val userPreferencesDao: UserPreferencesDao) {
    ...
    suspend fun saveUserPreferences(userPreferences: UserPreferences) {
        ...
        val merged = if (dbCurrent == null) {
            ...
            dbCurrent.copy(
                ...
                // ─── NEW: Merge weekly priorities — always keep whichever is more populated ───
                focusWeeklySubjectPriorities = mergePriorityMaps(
                    incoming.focusWeeklySubjectPriorities,
                    dbCurrent.focusWeeklySubjectPriorities
                ),
                casualWeeklySubjectPriorities = mergePriorityMaps(
                    incoming.casualWeeklySubjectPriorities,
                    dbCurrent.casualWeeklySubjectPriorities
                ),
    ...
    /**
     * Merges two weekly-priority maps.
     * Incoming wins for any key it contains; fall back to db for keys only in db.
     * This ensures a fresh week entry is never lost.
     */
    private fun mergePriorityMaps(
        incoming: Map<String, List<String>>,
        db: Map<String, List<String>>
    ): Map<String, List<String>> {
        if (incoming.isEmpty()) return db
        val merged = db.toMutableMap()
    ...
    suspend fun updateTaskCompletion(userId: String, taskId: String, isCompleted: Boolean, modeName: String) {
        ...
        val updatedPrefs = when (modeName) {
            "FOCUS" -> {
                ...
                currentPrefs.copy(tasks = updatedTasks, lastAccessed = System.currentTimeMillis())
            }
            "CASUAL" -> {
                val updatedCasualTasks = currentPrefs.casualTasks.map { task ->
                    if (task.id == taskId) task.copy(completed = isCompleted) else task
                }
                currentPrefs.copy(casualTasks = updatedCasualTasks, lastAccessed = System.currentTimeMillis())
            }
            else -> return
        }
